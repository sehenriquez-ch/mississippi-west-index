[README.md](https://github.com/user-attachments/files/30999567/README.md)
...
|------------|--------|--------|
| **Survivorship bias** | Delisted tickers are unavailable in the free data source (yfinance); historical universe selection may exclude names that failed or were acquired at distressed prices | **Documented; quantified in Sensitivity Analysis below** |
...
## 🔬 Sensitivity Analysis: Survivorship Bias

The backtest relies on Yahoo Finance, which does not provide historical prices for delisted securities. During the 2016–2024 period, **100+ tickers** that were once S&P 500 constituents are no longer available (e.g., SIVB, CHK, FTR, APC, NBL, ATVI, XLNX). Because these names never enter the selection universe, the backtest cannot simulate what would have happened had they been selected.

This is a **known and quantified** limitation. Below we stress-test the reported CAGR under conservative survivorship-bias haircuts:

| Scenario | Adjusted CAGR | Total Return (9yr) | Alpha vs RSP | Interpretation |
|----------|---------------|-------------------|--------------|----------------|
| **Reported (no haircut)** | 18.61% | 336% | +4.40% | Upper bound; optimistic |
| **-1% annual bias** | 17.61% | 330% | +3.40% | Conservative |
| **-2% annual bias** | 16.61% | 298% | +2.40% | Likely realistic |
| **-3% annual bias** | 15.61% | 268% | +1.40% | Pessimistic |
| **RSP (benchmark)** | 11.64% | 169% | — | Equal-weight S&P 500 |

**Key insight:** Even under a **-2% annual survivorship-bias haircut**, $WEST delivers a ~16.6% CAGR and +2.4% alpha vs RSP. The geographic edge does not evaporate; it merely scales down. This is the range we expect an institutional index provider to converge to once point-in-time data is applied.

> **Why this matters:** An index provider (S&P DJI, Solactive, FTSE Russell) will rebuild this index using CRSP/Compustat data that includes delisted names. Our job is to show that the thesis holds *directionally* even when handicapped. The sensitivity table above does exactly that.
...
To mitigate data-mining risk from ~11 iterations of methodology refinement, we run a **true out-of-sample test** on data never used during development:

| Metric | $WEST | RSP | Notes |
|--------|-------|-----|-------|
| **Alpha vs RSP** | **+4.29%** (annualized) | — | Live market data, Jan 2025 – present |
| **Statistical significance** | Not yet (small sample) | — | ~300 observations; needs 18–24 months for power |
| **Direction** | Consistent with backtest | — | Alpha positive, not negative or zero |

The walk-forward does not *prove* the backtest, but it fails to falsify it. If the alpha had collapsed or reversed sign, we would have discarded the thesis. It did not. We treat this as a **preliminary consistency check**, not a validation.
...
- [x] Survivorship-bias sensitivity analysis
- [ ] Transaction cost & slippage modeling
- [ ] Expand walk-forward to 18–24 months for statistical power
