#!/usr/bin/env python3
"""
Mississippi Divide West Index v11.4 — Public Proxy Explicit + HQ Hardening
=============================================================================
CAMBIOS v11.4 sobre v11.3:
0. Benchmark de mercado: ^GSPC (price return) -> ^SP500TR (S&P 500 Total Return)


1. DATA_MODE = "public_proxy" documentado en codigo (shares/HQ/free-float no
   son point-in-time; survivorship bias presente).
2. Comentario explicito de look-ahead en clasificacion HQ (Wikipedia actual +
   correcciones manuales, NO base point-in-time completa).
3. HQ_CORRECTIONS ampliado (HPE, CVX) y SEDES_HISTORICAS ampliado (SIVB, FRC,
   APC, CXO, TWTR, SBNY, etc.) to reduce Neutral fall-through for delisted names.

CAMBIOS v11.3 sobre v11.2:
1. Index level = cumulative product of equal-weight returns (base 1000).
2. kurtosis_diagnostic() for outlier analysis.
"""
...
try:
    ...
    print("WARNING: statsmodels not installed. Run: !pip install statsmodels -q")
    print("   Without statsmodels, Newey-West adjustment cannot be computed;")
    print("   falling back to plain OLS (less rigorous).")

warnings.filterwarnings('ignore')

# ================================================================
# DATA MODE — RESEARCH PROXY
# ================================================================
# This backtest runs in PUBLIC_PROXY mode.
# Implications:
#   - Shares outstanding: current (fast_info), NOT point-in-time
#   - Free-float: estimated proxy, NOT institutional
#   - HQ classification: mostly current Wikipedia + manual corrections
#     (NOT full point-in-time historical HQ)
#   - Delisted tickers: often missing from yfinance → survivorship bias
#   - Prices/volumes: Yahoo Finance (public)
#
# An institutional version would require CRSP/Compustat (or equivalent)
# for point-in-time shares, free-float, delisted prices, and historical HQ.
# ================================================================

DATA_MODE = "public_proxy"   # Do not change unless you wire institutional data
...
MAPA_ESTADOS = {
    'NY':'East','NJ':'East','PA':'East','MA':'East','CT':'East','MD':'East',
    'VA':'East','NC':'East','SC':'East','GA':'East','FL':'East','OH':'East',
    'IL':'East','MI':'East','IN':'East','WI':'East','KY':'East','TN':'East',
    'WV':'East','DE':'East','VT':'East','NH':'East','ME':'East','RI':'East',
    'AL':'East','MS':'East','DC':'East',
    'CA':'West','WA':'West','OR':'West','TX':'West','AZ':'West','CO':'West',
    'UT':'West','NV':'West','ID':'West','MT':'West','WY':'West','NM':'West',
    'AK':'West','HI':'West','MN':'West','IA':'West','MO':'West','AR':'West',
    'LA':'West','ND':'West','SD':'West','NE':'West','KS':'West','OK':'West',
    'New York':'East','New Jersey':'East','Pennsylvania':'East',
    'Massachusetts':'East','Connecticut':'East','Maryland':'East',
    'Virginia':'East','North Carolina':'East','South Carolina':'East',
    'Georgia':'East','Florida':'East','Ohio':'East','Illinois':'East',
    'Michigan':'East','Indiana':'East','Wisconsin':'East','Kentucky':'East',
    'Tennessee':'East','West Virginia':'East','Delaware':'East',
    'Vermont':'East','New Hampshire':'East','Maine':'East','Rhode Island':'East',
    'Alabama':'East','Mississippi':'East','District of Columbia':'East',
    'California':'West','Washington':'West','Oregon':'West','Texas':'West',
    'Arizona':'West','Colorado':'West','Utah':'West','Nevada':'West',
    'Idaho':'West','Montana':'West','Wyoming':'West','New Mexico':'West',
    'Alaska':'West','Hawaii':'West','Minnesota':'West','Iowa':'West',
    'Missouri':'West','Arkansas':'West','Louisiana':'West',
    'North Dakota':'West','South Dakota':'West','Nebraska':'West',
    'Kansas':'West','Oklahoma':'West',
}

# ---------------------------------------------------------------
# HQ CLASSIFICATION — IMPORTANT LIMITATION
# ---------------------------------------------------------------
# base_hq_map is built primarily from the CURRENT Wikipedia S&P 500 page.
# That means most tickers receive their 2025/2026 headquarters for the
# entire 2016–2024 history (look-ahead in geographic classification).
#
# HQ_CORRECTIONS below only patches a limited set of well-documented moves.
# SEDES_HISTORICAS_* covers some delisted names.
# This is NOT a full point-in-time HQ database.
# ---------------------------------------------------------------

HQ_CORRECTIONS = {
    # Format: ticker -> list of (start_date or None, end_date or None, state)
    ...
    'HPE':  [(None, '2020-12-01', 'CA'), ('2020-12-01', None, 'TX')],
    'SCHW': [(None, '2021-01-01', 'CA'), ('2021-01-01', None, 'TX')],
    'CBRE': [(None, '2020-01-01', 'CA'), ('2020-01-01', None, 'TX')],
    'TMUS': [(None, None, 'WA')],
    # Chevron announced HQ move CA → TX in Aug 2024 (near/after sample end)
    'CVX':  [(None, '2024-08-01', 'CA'), ('2024-08-01', None, 'TX')],
}
...
SEDES_HISTORICAS_OESTE = {
    ...
    # Extra delisted / high-impact West names (reduce Neutral fall-through)
    'SIVB':'CA',   # Silicon Valley Bank
    'FRC':'CA',    # First Republic
    'CXO':'TX',
    'APC':'TX',    # Anadarko
    'KSU':'KS',
    'TWTR':'CA',
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                if attempt == self.MAX_RETRIES - 1:
                    raise DataProviderError(f"Failed after {self.MAX_RETRIES} attempts: {e}")
    ...
    def get_historical_prices(self, tickers, start, end):
        ...
        if self.cache:
            ...
            if cached is not None:
                logging.info("Using prices from cache")
                return cached

        logging.info(f"Downloading {len(tickers)} tickers in batches of {self.BATCH_SIZE}...")
        logging.info("NOTE: Delisted tickers will generate warnings. This is NORMAL.")
        ...
        for i in range(0, len(tickers_list), self.BATCH_SIZE):
            ...
            try:
                ...
                logging.warning(f"  Batch {batch_num} failed entirely ({e}), retrying ticker by ticker...")
                for t in batch:
                    try:
        ...
        if self._delisted_tickers:
            logging.warning(f"Possibly delisted tickers: {len(self._delisted_tickers)}")
        return result
...
class IndexCalculationEngine:
    ...
    def calculate_index(self, label, df_close, df_volume, df_bench,
        ...
        """
        FIX v11.3: Level is now derived from cumulative equal-weight returns,
        eliminando el desalineamiento con el divisor market-cap.
        """
        logging.info(f"Computing index {label}...")
        ...
        # CRITICAL FIX v11.3: Level = cumulative equal-weight returns, base 1000
...
def comparative_analysis(west_returns: pd.Series, qqq_returns: pd.Series,
    ...
    for name, bench_returns in [("QQQ", qqq_returns), ("RSP", rsp_returns)]:
        ...
        if HAS_STATSMODELS:
            try:
                ...
                logging.warning(f"Newey-West failed for {name}: {e}")
...
def kurtosis_diagnostic(returns: pd.Series, label: str = "$WEST"):
    """
    Identify the source of elevated kurtosis in daily returns.
    Run after main() to understand what drives fat tails.
    """
    ...
    print("=" * 70)
    print(f"  KURTOSIS DIAGNOSTIC — {label}")
    print("=" * 70)
    print(f"  Observations: {len(rets)}")
    print(f"  Total kurtosis: {rets.kurtosis():.2f}")
    print(f"  Skewness: {rets.skew():.2f}")
    print()

    print("  TOP 15 MOST NEGATIVE RETURNS:")
    for date, val in rets.nsmallest(15).items():
        print(f"    {date.strftime('%Y-%m-%d')}: {val*100:+.2f}%")
    print()

    print("  TOP 15 MOST POSITIVE RETURNS:")
    ...
    eventos = {
        ...
        "COVID Rally (Apr 2020)": ("2020-04-01", "2020-04-30"),
        "US Election (Nov 2020)": ("2020-11-01", "2020-11-30"),
        "Inflation/Russia (Feb-Mar 2022)": ("2022-02-01", "2022-03-31"),
    ...
    for nombre, (ini, fin) in eventos.items():
        ...
        if len(subset) > 0:
            excluido = rets[~mask]
            print(f"  {nombre}:")
            print(f"    Days: {len(subset)}, Min: {subset.min()*100:.2f}%, Max: {subset.max()*100:.2f}%")
            print(f"    Kurtosis WITHOUT this period: {excluido.kurtosis():.2f}")
            print()
    ...
    if rets_sin_eventos.kurtosis() > 8:
        print(f"  WARNING: Kurtosis still high ({rets_sin_eventos.kurtosis():.2f}) without known events.")
        print("     Possible cause: delisted tickers with extreme gaps or data errors.")
        print("     Review isolated dates with returns < -10% or > +10% outside listed events.")
    else:
        print(f"  OK: Kurtosis mostly explained by market events ({rets_sin_eventos.kurtosis():.2f} without events).")
...
def main(start_date=None, end_date=None, no_cache=False):
    ...
    logger.info("Mississippi Divide West Index v11.4 — SP500TR benchmark + English UI")
    logger.info(f"DATA_MODE = {DATA_MODE}")
    if DATA_MODE == "public_proxy":
        logger.warning(
            "Running in PUBLIC_PROXY mode: shares/HQ/free-float are not point-in-time. "
            "Survivorship bias from missing delisted prices is present. "
            "See README sensitivity analysis."
        )
    ...
    logger.info(f"Period: {config.start} -> {config.end}")
    ...
    if "^SP500TR" not in df_close.columns:
        logger.error("Missing ^SP500TR (S&P 500 Total Return)"); return 1

    df_bench = df_close["^SP500TR"].pct_change().fillna(0.0)
    ...
    logger.info("Downloading shares outstanding (proxy)...")
    shares_map = provider.get_shares_outstanding(list(all_tickers - {"^SP500TR", "QQQ", "RSP"}))
    logger.warning("Current shares used as proxy. Production requires Compustat/CRSP.")

    logger.info("Computing free-float factors...")
    free_float_map = {}
    for ticker in all_tickers - {"^SP500TR", "QQQ", "RSP"}:
        shares = shares_map.get(ticker, 1e8)
    ...
    logger.info(f"Free float computed for {len(free_float_map)} tickers")
    ...
    sp500_m = bench_stats("^SP500TR")
    qqq_m = bench_stats("QQQ")
    rsp_m = bench_stats("RSP")

    comparativo = comparative_analysis(res_west["returns"], df_qqq_ret, df_rsp_ret, config.rf_annual)

    w = res_west["metrics"]
    print("\n" + "=" * 110)
    print("  MISSISSIPPI DIVIDE WEST INDEX v11.4 — INVESTOR PRESENTATION EDITION (EQUAL WEIGHT)")
    print("=" * 110)
    print(f"  Period: {config.start} -> {config.end}  |  Base Level: 1,000  |  Equal Weight (1/N)")
    print("-" * 110)

    print(f"\n  {'METRIC':<28} {'WEST':>14} {'S&P500TR':>14} {'QQQ':>14} {'RSP (EW)':>14}")
    print("  " + "-" * 78)
    metrics_rows = [
        ("CAGR", f"{w['cagr']:>13.2f}%", f"{sp500_m['cagr']:>13.2f}%", f"{qqq_m['cagr']:>13.2f}%", f"{rsp_m['cagr']:>13.2f}%"),
        ("Annual Volatility", f"{w['volatility']:>13.2f}%", f"{sp500_m['volatility']:>13.2f}%", f"{qqq_m['volatility']:>13.2f}%", f"{rsp_m['volatility']:>13.2f}%"),
        ...
        ("Alpha vs S&P500TR", f"{w['alpha']:>13.2f}%", "—", "—", "—"),
        ("Beta vs S&P500TR", f"{w['beta']:>14.2f}", "1.00", "—", "—"),
    ...
    print("\n" + "=" * 70)
    print("  IS $WEST DIFFERENT FROM QQQ AND RSP? (correlation + alpha regression)")
    print("=" * 70)
    for name in ["QQQ", "RSP"]:
        comp = comparativo.get(name)
        if comp is None:
            print(f"\n  vs {name}: not available")
            continue
        ...
        print(f"    Annual alpha: {comp['alpha_annual_pct']:+.2f}%")
        print(f"    p-value plain OLS:     {comp['p_value_alpha_ols']:.4f}")
        if comp['p_value_alpha_nw'] is not None:
            print(f"    p-value Newey-West:    {comp['p_value_alpha_nw']:.4f}  <- THE ONE THAT MATTERS")
        else:
            print(f"    p-value Newey-West:    not available")
        if comp["alpha_significant_nw"]:
            print(f"    OK: Alpha SIGNIFICANT even with Newey-West — solid evidence of a real edge")
        elif comp["alpha_significant_ols"]:
            print(f"    WARNING: Alpha significant under plain OLS BUT NOT Newey-West —")
            print(f"       autocorrelation was inflating confidence. Treat with caution.")
        else:
            print(f"    WARNING: Alpha NOT significant under either test.")
    ...
    print(f"  {'Year':<6} {'WEST':>10} {'S&P500TR':>10} {'QQQ':>10} {'RSP':>10}  Winner")
    print("  " + "-" * 60)
    for yr in sorted(w["yearly_returns"].index):
        ...
        winner = "WEST" if west_yr > sp_yr else "S&P TR"
    ...
    if res_west["turnover_history"]:
        ...
        print(f"\n  TURNOVER ANALYSIS")
        print(f"  Average per rebalance:    {avg_turnover*100:.2f}%")
        print(f"  Maximum per rebalance:    {max_turnover*100:.2f}%")
        print(f"  Rebalances executed:     {len(res_west['turnover_history'])}")

    if w.get("recovery_times"):
        print(f"\n  DRAWDOWN RECOVERY ANALYSIS")
        print(f"  Number of drawdowns >1%: {len(w['recovery_times'])}")
    ...
    print("\n")
    kurtosis_diagnostic(res_west["returns"], label="$WEST")
    ...
    # Save index levels for reference
    ...
    ax1.plot(cum_s, color='#8B8B8B', linewidth=1.8, linestyle='--', label='S&P 500 TR')
    ...
    ax2.fill_between(dd_s.index, dd_s, 0, alpha=0.15, color='#8B8B8B', label='S&P 500 TR')
    ...
    ax3.plot(rs_s, color='#8B8B8B', linewidth=1.2, linestyle='--', label='S&P 500 TR')
    ...
    rb_w = w["rolling_beta"]
    ax4.plot(rb_w, color='#FF8C42', linewidth=1.5, label='$WEST vs S&P 500 TR')
    ax4.axhline(y=1.0, color='white', alpha=0.3, linewidth=0.5, linestyle='--')
    ax4.set_title('Rolling 63-Day Beta vs S&P 500 TR', color='white', fontsize=12)
    ...
    def comp_block(name, comp):
        ...
        if comp["alpha_significant_nw"]:
            veredicto = "YES, even with Newey-West (HAC) — solid evidence"
        elif comp["alpha_significant_ols"]:
            veredicto = "Significant under plain OLS but NOT Newey-West — likely inflated by autocorrelation; treat with caution"
        else:
            veredicto = "NOT confirmed under either test"
        return f"""### vs {name}

- Daily correlation: {comp['correlation']:.4f}
- Beta: {comp['beta']:.3f}
- R²: {comp['r_squared']:.3f}
- Annualized alpha: {comp['alpha_annual_pct']:+.2f}%
- p-value (plain OLS): {comp['p_value_alpha_ols']:.4f}
- p-value (Newey-West, {comp['nw_lags']} lags): {p_nw_str}
- **Verdict: {veredicto}**
"""

    report_md = f"""# Mississippi Divide West Index v11.3
## Executive Summary for Investor Presentation (Equal Weight, with analysis vs RSP/QQQ)

**Period:** {config.start} to {config.end}
**Weighting:** Equal Weight (1/N) — switched from float-adjusted market cap
due to excessive megacap tech concentration (see limitations section).
**Rebalancing:** Quarterly (3rd Friday of Mar/Jun/Sep/Dec)
**Universe:** S&P 500 constituents with HQ West of the Mississippi River

---

## 0. Known limitations (read first)

1. Shares outstanding: current proxy, not historical point-in-time.
2. Free float: estimated, not measured with institutional data.
3. Comparative alpha section reports both plain OLS and Newey-West p-values;
   prefer Newey-West. Treat results as indicative under public_proxy mode.
4. Methodology iterated ~11 times during development — overfitting risk
   not ruled out without out-of-sample validation (2025-26).
5. **FIX v11.3/v11.4:** Index level is consistent with equal-weight returns
   (previously a market-cap divisor caused misalignment). Benchmark is ^SP500TR.
...
## 2. Is $WEST different from QQQ and RSP? (what decides everything)
...
- Average turnover per rebalance: {avg_turnover*100:.2f}%
- Maximum turnover: {max_turnover*100:.2f}%
- Drawdowns >1%: {len(w.get('recovery_times', []))}
- Average recovery time: {np.mean([rt[2] for rt in w.get('recovery_times', [])]) if w.get('recovery_times') else float('nan'):.0f} days

---

*Disclaimer: Preliminary backtest for discussion with index providers.
Not suitable for final investor distribution without addressing section 0.*
"""
    ...
    print("\n  OK v11.4: Index level is consistent with equal-weight returns; benchmark = S&P 500 TR.")
    logger.info("Index calculation v11.4 completed")
    return 0
